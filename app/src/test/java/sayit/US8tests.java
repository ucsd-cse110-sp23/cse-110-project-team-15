...
public class US8tests {
    ...
    @Test
    void createEmailTest() throws IOException, InterruptedException {
        ...
        String serverSubject = response.substring(response.indexOf("/C\\") + 3, response.indexOf("/D\\"));
        String serverBody = response.substring(response.indexOf("/D\\") + 3);

        assertEquals("Create Email", command);
        assertEquals("this is a subject", serverSubject);
        assertEquals("This is the mock answer!!! :)", serverBody);
    }
}
