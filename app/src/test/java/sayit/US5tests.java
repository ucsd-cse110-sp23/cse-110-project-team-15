...
public class US5tests {
    ...
    @Test
    void newQuestionTest() throws IOException, InterruptedException {
        ...
        String serverQuestion = response.substring(response.indexOf("/C\\") + 3, response.indexOf("/D\\"));
        String serverAnswer = response.substring(response.indexOf("/D\\") + 3);

        assertEquals("New Question", command);
        assertEquals("this is a question", serverQuestion);
        assertEquals("This is the mock answer!!! :)", serverAnswer);
    }
}
